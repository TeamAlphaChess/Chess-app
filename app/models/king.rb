class King < Piece
	...
	def valid_move?(destination_row, destination_col, color)
    ...
    # Check to see if the king is only moving exactly one space in any direction on the board
 		!invalid_destination?(destination_row, destination_col) && distance(destination_row, destination_col) == 1 && !same_color(color)
 	end
end
